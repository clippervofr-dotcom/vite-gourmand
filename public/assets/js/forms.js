...
if (btnDevis && devisModal && devisForm) {
    ...
    devisForm.addEventListener('submit', async function (event) {
        ...
        try {
            const reponse = await fetch('/contact/traitement-devis.php', {method: 'POST', body: donnees});
...
if (contactForm) {
    contactForm.addEventListener('submit', async function (event) {
        ...
        try {
            const reponse = await fetch('/contact/traitement-contact.php', {method: 'POST', body: donnees});
            const reponseDonnees = await reponse.json();
