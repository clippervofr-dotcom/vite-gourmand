...
class HistoriqueStatutRepositoryMysql implements HistoriqueStatutRepositoryInterface
{
    ...
    {
        $sql = 'SELECT * FROM historique_statut WHERE historique_statut_id = :historique_statut_id';
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':historique_statut_id', $historiqueStatutId, PDO::PARAM_INT);
        $stmt->execute();
        $ligne = $stmt->fetch();
        if ($ligne === false) {
            return null;
    ...
    public function save(HistoriqueStatut $historiqueStatut): void
    {
        if ($historiqueStatut->getHistoriqueStatutId() === null) {
            ...
            $sql = 'UPDATE historique_statut SET commande_id = :commande_id, statut = :statut, date_changement = :date_changement WHERE historique_statut_id = :historique_statut_id';
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindValue(':commande_id', $historiqueStatut->getCommandeId(), PDO::PARAM_INT);
            $stmt->bindValue(':statut', $historiqueStatut->getStatut(), PDO::PARAM_STR);
            $stmt->bindValue(':date_changement', $historiqueStatut->getDateChangementStatut(), PDO::PARAM_STR);
            $stmt->bindValue(':historique_statut_id', $historiqueStatut->getHistoriqueStatutId(), PDO::PARAM_INT);
            $stmt->execute();
        }
    }
    ...
    {
        $sql = 'DELETE FROM historique_statut WHERE historique_statut_id = :historique_statut_id';
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':historique_statut_id', $historiqueStatutId, PDO::PARAM_INT);
        $stmt->execute();
    }
    ...
    private function mapLigneVersHistoriqueStatut(array $ligne): HistoriqueStatut
    {
        return new HistoriqueStatut(
            historiqueStatutId: $ligne['historique_statut_id'],
