...
if (conteneurListeEmploye) {
    ...
    async function chargerListeEmploye() {
        const reponse = await fetch('/profil/profil-admin-liste-employe.php');
    ...
    async function creationEmploye() {
        ...
        try {
            const reponse = await fetch('/profil/profil-admin-creation-employe.php', {
                method: 'POST',
                body: donnees
            })
            const nouvelleListeEmploye = await reponse.json();
            ...
            console.error('Erreur du fetch des employés :', erreur);
        }
    }
    ...
    btnValiderCreation.addEventListener('click', function (event) {
        creationEmploye().catch(function (erreur) {
            console.error('Erreur lors de la création de l\'employé :', erreur);
        });
    });
...
if (conteneurModifHoraires) {
    ...
    async function chargerHoraires() {
        const reponse = await fetch('/horaires/horaires-chargement.php');
    ...
    btnValiderHoraires.addEventListener('click', function () {
        async function nouveauxHoraires() {
            ...
            await fetch('/horaires/changement-horaires.php', {
                method: 'POST',
                body: donnees
            });
