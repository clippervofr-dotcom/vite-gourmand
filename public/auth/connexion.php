...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $cleTentatives = 'tentatives_connexion_' . md5((string)$email);
    $_SESSION[$cleTentatives] = $_SESSION[$cleTentatives] ?? ['nb' => 0, 'derniere' => 0];

    if ($_SESSION[$cleTentatives]['nb'] >= 5 && time() - $_SESSION[$cleTentatives]['derniere'] < 300) {
        ...
        if (!$utilisateur) {
            ...
            if ($verification) {
                unset($_SESSION[$cleTentatives]);
                session_regenerate_id(true);
                $_SESSION['utilisateur'] = $utilisateur->jsonSerialize();
                header('Location: /index.php');
                exit;
            } else {
                $_SESSION[$cleTentatives]['nb']++;
                $_SESSION[$cleTentatives]['derniere'] = time();
                $erreur = 'Email ou mot de passe incorrect';
            }
        }
    }
