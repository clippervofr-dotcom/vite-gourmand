...
async function chargerCommandesEnAttente(statut) {
    const reponse = await fetch('/commandes/commandes-en-attente.php?statut=' + encodeURIComponent(statut));
...
if (conteneurCommandes) {
    ...
    conteneurCommandes.addEventListener('click', async function (event) {
        ...
        try {
            const reponse = await fetch('/commandes/commandes-employe-valider.php', {
                method: 'POST',
                body: donnees
            });
            ...
            console.error('Erreur lors de la validation de la commande :', erreur);
        }
    ...
    conteneurCommandes.addEventListener('click', async function (event) {
        ...
        try {
            const reponse = await fetch('/commandes/profil-voir-details-commandes.php', {
                method: 'POST',
                body: donnees
            });
            const resultat = await reponse.json();
            ...
            console.error('Erreur lors de la récupération des détails de la commande :', erreur);
        }
...
if (annulationModalAdmin && annulationModalAdminClose) {
    ...
    annulationModalAdmin.addEventListener('click', async function (event) {
        ...
        try {
            const reponse = await fetch('/commandes/commandes-admin-annuler.php', {
                method: 'POST',
                body: donnees
            });
            const resultat = await reponse.json();
